...
const cv::Mat kCameraIntrinsics =  (cv::Mat_<double>(3,
                                                     3) << kFocalLengthColorX, 0, kPrincipalPointX,
...
// default.
std::vector<PointValuePair> GetLocalMaxima(const cv::Mat &image,
                                           bool sort = true);
...
void WrapVector2DToCVMat(std::vector<std::vector<double>> &array2d,
                         cv::Mat &mat);

// This copies data. Therefore, changes to the returned cv::Mat
// will affect the vector2d array.
void Vector2DToCVMat(const std::vector<std::vector<double>> &array2d,
                     cv::Mat &mat);
